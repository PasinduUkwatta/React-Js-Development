...
def address():
    ...
    if request.method == 'POST':
        ...
        sql = "SELECT email FROM address Where email=%s"
        data_search = (email,)
        ...
        if results:
            return jsonify(
            {'message': "This Email Already Contain Address Details ",
             "code":"409"})


        else:
            query = "INSERT INTO  address(line1,line2,postal_code,city,state,country,email) VALUES (%s,%s,%s,%s,%s,%s,%s)"
            val = (line1, line2, postalcode, city, state, country,email)
            mycursor.execute(query, val)
...
def business():
    ...
    if request.method == 'POST':
        ...
        sql = "SELECT email FROM business Where email=%s"
        data_search = (email,)
        ...
        if results:
            return jsonify(
            {'message': "This Email Already Contain business Details ",
             "code":"409"})

        else:

            query = "INSERT INTO  business(business_name,business_owner_name,business_reg_no,contact_no,email) VALUES (%s,%s,%s,%s,%s)"
            val = (businessname, businessownername, businessregno, contactno,email)
            mycursor.execute(query, val)
            connection.commit()
            return jsonify({'result': result})
...
def user_details():
    ...
    if request.method == 'POST':
        ...
        sql2 = "SELECT * FROM address Where email=%s"
        sql3 = "SELECT * FROM business Where email=%s"


        data_search = (email,)

        mycursor.execute(sql1, data_search)
        results1 = mycursor.fetchall()

        mycursor.execute(sql2, data_search)
        results2 = mycursor.fetchall()

        mycursor.execute(sql3, data_search)
        results3 = mycursor.fetchall()

        connection.commit()
        return jsonify({'user_results': results1},{'address_results' :results2},{'business_results' :results3})

@app.route('/user-payment-details', methods=['POST'])
@app.route('/admin/user-payment-details', methods=['POST'])

def user_payment_details():

    if request.method == 'POST':
        user_payment_details = request.get_json()
        email = user_payment_details['email']
        connection = mysql.connector.connect(host=HOST_NAME, user=USER_NAME, password=PASSWORD,database=DATABASE)
        mycursor = connection.cursor()
...
def protected():
    ...
    return jsonify({'message': "Okey","user_email":current_user}), 200
